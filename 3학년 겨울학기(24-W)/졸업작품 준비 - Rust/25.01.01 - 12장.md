...
- 프로그램을 _main.rs_ 와 _lib.rs_ 로 분리하고 프로그램 로직을 _lib.rs_ 로 옮기세요.
- 커맨드 라인 파싱 로직이 작은 동안에는 _main.rs_ 에 남을 수 있습니다.
- 커맨드 라인 파싱 로직이 복잡해지기 시작하면, _main.rs_ 로부터 추출하여 _lib.rs_ 로 옮기세요.