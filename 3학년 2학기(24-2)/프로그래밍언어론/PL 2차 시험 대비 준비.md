...
- 일부 문자열은 유효한 프로그램이고, 다른 문자열은 그렇지 않습니다. 
...
- 이 코드는 유효한 스칼라 프로그램입니다. 
- 그러나 다음 코드는 유효하지 않은 스칼라 프로그램입니다.

```scala
println(
```

- 구체적 구문은 특정 문자열이 프로그램인지 여부를 결정합니다. 
- 스칼라의 구체적 구문에 따르면, println()은 프로그램이지만 println(는 닫는 괄호가 부족하여 유효하지 않습니다. 
- 구체적 구문 없이는 프로그래머가 특정 문자열이 프로그램인지 여부를 알 수 없습니다. 
- 구체적 구문은 프로그래밍 언어의 필수 요소 중 하나입니다.

---
- 문자열을 정의하는 것이 첫 번째 단계입니다. 
- 문자열은 문자들의 유한한 시퀀스입니다. 
- 문자의 정의는 프로그래밍 언어마다 다를 수 있습니다.
- 일부 언어에서는 문자가 ASCII 코드로 표현되는 기호로 제한되지만, 다른 경우에는 모든 유니코드 기호를 문자로 간주합니다. 
- 우리는 일반적인 프로그래밍 언어를 다룰 것이므로 문자의 집합을 특정 집합으로 고정하지 않습니다. 
- 대신 특정 문자 집합이 주어진다고 가정하며, 해당 문자 집합에 어떤 문자가 존재하는지는 신경 쓰지 않습니다.
...
- 문자열의 정의는 프로그램의 정의와 다릅니다. 
- 어떤 문자열은 프로그램이 아닐 수 있습니다. 
- 예를 들어 "println()"은 프로그램이지만, "println("은 문자열이지만 프로그램이 아닙니다.