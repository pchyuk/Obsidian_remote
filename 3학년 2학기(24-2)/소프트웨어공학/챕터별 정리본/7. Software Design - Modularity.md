...
### 기능적 응집력 모듈(Functional Cohesion)은 모두 **하나의 문제 관련 작업**을 수행하는 데 기여하는 요소들을 포함한다.
...
### 순차적으로 응집력이 있는 모듈은 **한 활동의 출력 데이터가 다음 활동의 입력 데이터로 사용되도록** 요소가 활동에 관여하는 모듈이다.
...
### 순차적으로 응집력이 있는 모듈은 일반적으로 결합력이 좋고 쉽게 유지 관리할 수 있다.
