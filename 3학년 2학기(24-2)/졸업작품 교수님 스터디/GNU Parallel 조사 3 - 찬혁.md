...
- 레코드의 순서가 중요하지 않은 경우(예: 모든 줄을 처리해야 하지만 어떤 줄이 먼저 처리되는지는 중요하지 않은 경우) `--round-robin` 옵션을 사용할 수 있습니다. 
- `--round-robin` 옵션이 없을 경우, GNU Parallel은 블록당 하나의 명령을 실행합니다. 하지만 `--round-robin`을 사용하면 요청된 작업 수(-j 또는 --jobs)에 따라 실행됩니다. 레코드는 실행 중인 작업 사이에 분배됩니다:
...
4개의 인스턴스 중 1개는 단일 청크를, 2개는 각각 2개의 전체 청크를, 1개는 1개의 전체 청크와 1개의 일부 청크를 받았습니다.  
