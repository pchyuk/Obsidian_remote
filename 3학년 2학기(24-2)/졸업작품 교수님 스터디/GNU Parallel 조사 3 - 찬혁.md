...
1. 입력 데이터는 반드시 일반 파일이거나 블록 디바이스여야 한다.  
    (파이프나 FIFO는 사용할 수 없다.)
2. 입력은 `-a` 또는 `::::`로 지정해야 합니다.
3. `-L`, `-1`, `-N` 옵션은 사용할 수 없습니다.

그러나 `--recend`와 `--recstart`는 사용할 수 있어, 이를 기반으로 레코드를 나누는 것이 가능합니다.

---

#### 예제: `--pipepart`로 데이터 처리

다음 명령은 데이터를 3MB 단위로 나눠 처리합니다:
...
`428443 8444444 3000002 428572 428572 3000004 126985 126984 888890`

---

#### `--block`에 음수 값 지정하기

`--block`에 음수 값을 설정하면 각 작업 슬롯이 처리해야 할 블록의 수를 의미합니다.  
...
- 이 방식은 `--round-robin`보다 효율적입니다.  
    GNU Parallel이 데이터를 읽지 않고 직접 처리하기 때문입니다.
- 소수의 작업 슬롯으로도 대규모 데이터를 처리할 수 있습니다.

---

#### 입력 순서를 유지하기: `--keep-order`

`--keep-order`를 사용하면 입력 데이터의 순서와 동일하게 출력할 수 있습니다.  
`--round-robin` 방식에서는 입력이 섞이기 때문에 순서를 유지할 수 없습니다.
