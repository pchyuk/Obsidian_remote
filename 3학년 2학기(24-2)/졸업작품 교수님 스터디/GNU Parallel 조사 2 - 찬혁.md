...
## 7.1 동시 작업 수
- 동시 작업 수는 `--jobs` 또는 `-j`로 지정됩니다 (`-N0`는 단일 인수를 읽지만, sleep 1을 여러 번 병렬로 실행합니다):


/usr/bin/time parallel -N0 -j64 sleep 1 ::: num128
64개의 작업을 병렬로 실행하면, 128개의 sleep은 머신 속도에 따라 2-8초가 걸릴 것입니다.

기본적으로 --jobs는 CPU 코어 수와 같습니다. 따라서 다음과 같이 실행됩니다:


/usr/bin/time parallel -N0 sleep 1 ::: num128
이는 CPU 코어당 2개의 작업을 실행하는 데 걸리는 시간의 두 배가 걸려야 합니다:


/usr/bin/time parallel -N0 --jobs 200% sleep 1 ::: num128
--jobs 0은 가능한 한 많은 작업을 병렬로 실행합니다:
